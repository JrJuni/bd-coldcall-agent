...
Stores (`WorkspaceStore`, `TargetStore`, `InteractionStore`, `DiscoveryStore`, `NewsStore`, `RagSummaryStore`, `RunStore`) all consume `_orm.get_session_factory()`. Do NOT introduce new raw `sqlite3.connect` paths — anything new goes through the ORM. The Alembic chain is `0001 → 0006`; new tables get their own idempotent migration. The cutover from SQLite to Neon is automated by `scripts/migrate_sqlite_to_postgres.py`.

Meeting Intelligence (`src/meeting_intelligence/`) joined the canonical seam in Phase M (2026-05-19). All 8 models (`Meeting`, `MeetingParticipant`, `MeetingInsight`, `MeetingActionItem`, `MeetingSemanticEvent`, `SemanticEntity`, `SemanticEntityMention`, `SemanticRelationship`) subclass `src.api.orm.Base`; schema delivery is Alembic `0006_meeting_intelligence` in production and `Base.metadata.create_all` in test fixtures. The module's old `MeetingBase` / `database.py::_FACTORY_CACHE` / `create_meeting_schema()` helpers were removed — meeting routes now resolve sessions through `_orm.get_session_factory()` just like every other store. When adding the *next* entity module (Accounts / Contacts / Signals / etc.), copy this pattern, not the pre-Phase-M island layout.
