...
- **Phase M** — Meeting Intelligence merge (2026-05-19)
  - **M0 — base-class merge.** `src/meeting_intelligence/` 가 자체 `MeetingBase(DeclarativeBase)` 를 들고 별도 island 로 살아있던 상태를 Phase 13 canonical seam (`src/api/orm.py::Base`) 으로 흡수. 8개 모델 (`Meeting` / `MeetingParticipant` / `MeetingInsight` / `MeetingActionItem` / `MeetingSemanticEvent` / `SemanticEntity` / `SemanticEntityMention` / `SemanticRelationship`) 의 base 만 교체 — 컬럼 / FK CASCADE / `uq_semantic_entities_name_type` 정의는 byte-identical. `create_meeting_schema(engine)` 는 `Base.metadata.create_all(engine, tables=[…])` 로 *meeting 테이블만* 생성하도록 재작성해서 기존 caller (test fixture, route) 가 그대로 동작. baseline 커밋 (`03119bd`, island as-is) + M0 커밋 (`bb2a56c`, single-file diff) 으로 분리.
  - **M1 — Alembic 0006 + standalone schema/session 캐시 retire.** `alembic/versions/0006_meeting_intelligence.py` 신규 — 8 테이블 idempotent (`_existing_tables()` inspector 가드), dual-engine type 만 사용 (`sa.Text` / `sa.Integer` / `sa.true()` / `sa.text("0")`), FK CASCADE 와 unique constraint 모두 SQLite / Postgres 양쪽에서 동일 렌더링. `src/meeting_intelligence/database.py` (자체 `_FACTORY_CACHE`) 삭제. `routes.py::get_meeting_repository` 는 이제 `_orm.get_session_factory()` 직접 호출. `models.py::create_meeting_schema` 자체도 제거 — test fixture 는 `Base.metadata.create_all(engine)`, 마이그레이션 path 는 Alembic. `src/api/models/__init__.py` 에 `from src.meeting_intelligence import models as _meeting_models` 사이드이펙트 import 를 추가해서 autogenerate 가 meeting 테이블을 본다. 동시에 `scripts/migrate_sqlite_to_postgres.py` 에 8개 meeting 모델 import + topological order 로 `COPY_ORDER` 확장 → Neon cutover 시 meeting 데이터도 함께 이전. `tests/test_migrate_sqlite_to_postgres.py` 에 Meeting + Participant + Event + Entity + Mention round-trip 시드 + FK id 보존 어서트 추가 (3 행 → 8 행).
  - **M2 — FastAPI mount + dedicated route tests.** `src/api/app.py::create_app` 가 `app.include_router(meeting_routes.router, tags=["meetings"])` 로 meeting 라우터를 production app 에 마운트. 새 `tests/test_meeting_routes.py` 6건 — `TestClient(create_app())` + `tests/test_api_workspaces.py` 의 `_reset_env` fixture (API_SKIP_WARMUP + API_APP_DB tmp + reset_api_settings_cache + `_store.reset_stores`) 패턴 + StaticPool `:memory:` SQLite (TestClient 워커스레드 간 단일 connection 공유) + `monkeypatch.setattr(_orm, "get_session_factory", lambda *a, **k: factory)` + LLM stub. 구조적 필드만 어서트 (status / source_type / evidence presence / action-item shape / `relationship.source_event_id`) — LLM prose golden 비교 금지. test_meeting_intelligence.py 의 m4 라우트 테스트는 책임을 옮기고 제거.
  - **검증**: Conda Python (`~/miniconda3/envs/bd-coldcall/python.exe`) 로 `pytest -q` → **597 tests passing** (Phase 13 583 + Meeting Intelligence 14, M1 대비 +5). Alembic chain `0001 → 0006_meeting_intelligence` 까지 확장. 회귀 0.
  - **commits (origin/phase-m-meeting-m0)**: `03119bd` baseline → `bb2a56c` M0 → `236e68d` M1 → `67f7ef6` migrator follow-up → `3d77483` M2.
  - **out of scope (Phase M 본체)**: Notion DB writer for meetings (Phase 13.5 RFP promote 와 같이 "Notion writer wave" 로 묶음), MCP tool wrap (`analyze_meeting` / `search_meetings` / `list_action_items`), HTTP/SSE MCP transport, 다른 entity 모듈 (Accounts / Contacts / Opportunities / PoC Plans / Product Feedback / Signals), Phase 13→main 머지.
